(function (global) {
    ...
    const initCharacteristicsPanel = (panel) => {
        ...
        const typeHelper = panel.querySelector('[data-characteristics-helper]');
        ...
        const toggleFeatureGroups = (type) => {
            const normalizedType = (type || '').toLowerCase();
            let hasMatch = false;

            featureGroups.forEach((group) => {
                const isMatch = group.dataset.featureGroup === normalizedType;
            ...
            if (typeHelper) {
                typeHelper.textContent = hasMatch
                    ? 'Agrega las características específicas para tu tipo de propiedad.'
                    : 'Selecciona un tipo de propiedad para ver características específicas.';
            }
        };
        ...
        const applyCharacteristicsContext = (detail = {}) => {
            ...
            if (typeBadge) {
                typeBadge.textContent = detail.typeLabel || 'Tipo no definido';
            }

            if (heading) {
                heading.textContent = title.trim().length ? `Características para: ${title}` : 'Define las características';
            }

            toggleFeatureGroups(detail.type || panel.dataset.publishType || '');
        };
        ...
        if (finishButton) {
            finishButton.addEventListener('click', (event) => {
                event.preventDefault();
                panel.classList.add('characteristics-card--saved');
            });
        }
